        ...
        try
        {
          while (true)
          {
            ...
            try
            {
              ...
              SDEIssue sdeIssue = null;

              if (taskMsg.Topic.Equals(CREATE_TOPIC_NAME))
              {
                sdeIssue = await integrationClient.CreateIssue(taskMsg.Value);
              }
              else if (taskMsg.Topic.Equals(UPDATE_TOPIC_NAME))
              {
                sdeIssue = await integrationClient.UpdateIssue(taskMsg.Value);
              }
              else if (taskMsg.Topic.Equals(REMOVE_TOPIC_NAME))
              {
                sdeIssue = await integrationClient.RemoveIssue(taskMsg.Value);
              }
